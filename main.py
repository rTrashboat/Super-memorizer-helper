...
def get_fonts():
    try:
        ...
        body_font = ("Mothanna.ttf", 15)
        button_font = ("VIMH.otf", 12, 'bold')
    except:
        # Fallbacks if custom fonts are not installed
    ...
    return title_font, body_font, button_font

def save_lesson_func(title, content, window_new):
    title_text = title.get()
    content_text = content.get("1.0", "end-1c").strip()
    if not title_text or not content_text:
        messagebox.showwarning("Missing Info", "Please fill both Title and Content fields.")
    else:
        title_text = title.get()
...
def Memorize_lesson(window_main, content, title, df):
    ...
    title_entry = Entry(window_new, font=body_font, relief=RAISED, bd=7)

    content_text = Label(window_new, text="Content:", bg="#347deb", fg="white", font=("VIMH.otf", 15, 'bold'), pady=3)
    content_entry = Text(window_new, font=body_font, relief=RAISED, bd=7)

    save_button = Button(button_frame, text="Save lesson", bg="#345beb", activebackground="#345beb", fg="white", 
                     activeforeground="white", font=button_font, padx=0, pady=8, width=90,height=1,
    ...
    return_button = Button(button_frame, text="⟵ ", bg="#345beb", activebackground="#345beb", fg="white", 
                     activeforeground="white", font=button_font, padx=0, pady=8, width=10,height=1,
                     command=lambda: read_lesson(df, title, window_new))

    button_frame.pack()
    return_button.pack(side="left", padx=5, pady=2.5)
    save_button.pack(side="right", padx=5, pady=2.5)
    title_text.pack()
...
def new_lesson_func(window_main):
    ...
    return_button = Button(button_frame, text="⟵ ", bg="#345beb", activebackground="#345beb", fg="white", 
                     activeforeground="white", font=button_font, padx=0, pady=8, width=10,height=1,
                     command=lambda: main(window_new))
...
def main(window=None):
    try:
        window.destroy()
        print("window before main destroyed")
    except Exception:
        print("Window is None type")
    global title_font
    global body_font
    global button_font
    title_font, body_font,button_font = get_fonts()
    window_main = Tk()
